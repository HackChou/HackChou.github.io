...
function count(valueA, valueB, operator) {
    ...
    switch (operator) {
        case '+':
            return accAdd(Number(valueA), Number(valueB));
        case '-':
            return accSubtr(Number(valueA), Number(valueB));
        case '*':
            return accMul(Number(valueA), Number(valueB));
        case '/':
            return accDiv(Number(valueA), Number(valueB));
        default:
            return 0;
    }
}
